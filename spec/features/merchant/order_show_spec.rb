...
RSpec.describe 'Merchant Order Show Page' do
  describe 'As a Merchant' do
    before :each do
      ...
      @order_1 = @m_user.orders.create!(status: "pending", address_id: @m_user_work.id)
    ...
    it 'I can see order information with only my order items' do
      ...
      expect(page).to have_content(@m_user_work.address)
      expect(page).to have_content(@m_user_work.city)
      expect(page).to have_content(@m_user_work.state)
      expect(page).to have_content(@m_user_work.zip)
